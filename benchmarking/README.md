...
# Run the lightweight H1-G0 through H1-G4 Viterbi-objective-vs-greedy smoke
...
### Public-Record Artifact Layout

Benchmark harnesses that emit public-record artifacts should write into a stable
directory layout:

- **Tables**: `tables/*.csv` with `benchmark_*` provenance columns prepended
- **Plots**: `plots/` for figures generated by the same run
- **Logs**: `logs/` for captured command output or execution logs
- **Provenance**: `provenance/*.json` sidecars plus `provenance/run.provenance.json`
- **Index**: `artifact-index.json` with relative paths for the run

The Rhizomorph dry-run harness can write a CI-sized smoke-testable record without
downloading data or executing heavy benchmark slices:

```bash
julia --project=. benchmarking/rhizomorph_benchmark_harness.jl \
  --plan --scale ci --write-artifacts --output-dir results/public-record
```
