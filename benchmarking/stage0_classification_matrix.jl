...
function main()
    ...
    # :full retains per-observation quality so get_vertex_mean_quality yields the
    # depth-independent per-base mean the fused arms need (review C1); the
    # lightweight profile only stores accumulated joint quality.
