    ...
    if run_external
        mktempdir() do _
            ...
            try
                ...
                expected_sim = joinpath(
                    dirname(asm_sim), replace(basename(asm_sim), Mycelia.FASTA_REGEX => "") *
                                      "_quast")
            ...
            try
                ...
                expected_real = joinpath(dirname(asm_real),
                    replace(basename(asm_real), Mycelia.FASTA_REGEX => "") * "_quast")
...
Test.@testset "BUSCO wrapper" begin
    ...
    """
    Test.@test Mycelia._parse_busco_dataset_list(sample_list_output) ==
        ...
        mktempdir() do _
            ...
            try
                if genome_info.source != :ncbi
                    ...
                    result_dir = Mycelia.run_busco(
                        [asm]; force = true, lineage = "bacteria_odb10", auto_lineage = false)
                    expected_outdir = joinpath(
                        dirname(asm), replace(basename(asm), Mycelia.FASTA_REGEX => "") *
                                      "_busco")
