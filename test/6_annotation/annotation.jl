...
Test.@testset "Annotation & feature extraction" begin
    Test.@testset "generate_transterm_coordinates_from_fasta" begin
        mktempdir() do dir
            ...
            rows = [split(strip(line))
                    for line in readlines(coords_file) if !isempty(strip(line))]
            ...
            Test.@test rows_by_gene["seq1_stop"][2:4] ==
                       [string(length("ATGCGT") - 1), string(length("ATGCGT")), "seq1"]
        end
    end
    ...
    Test.@testset "update_gff_with_mmseqs" begin
        mktempdir() do dir
            ...
            df = DataFrames.DataFrame(
                query = ["gene1"], qheader = ["gene1 # ID=gene1"], qlen = [5],
                target = ["t"], tlen = [5], theader = ["desc protein"],
                nident = [5], fident = [1.0], pident = [100.0],
                alnlen = [5], mismatch = [0], gapopen = [0], qstart = [1],
                qend = [5], tstart = [1], tend = [5],
