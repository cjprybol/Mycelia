...
#
# ```bash
# julia --project=. -e 'include("test/2_preprocessing_qc/constants.jl")'
# ```
#
...
Test.@testset "Constants" begin
    ...
    Test.@testset "Alphabet constants" begin
        ...
        Test.@testset "Size ordering" begin
            ...
            Test.@test length(Mycelia.UNAMBIGUOUS_DNA_CHARSET) ==
                       length(Mycelia.UNAMBIGUOUS_RNA_CHARSET) == 8

            ## ACGTN should be larger than unambiguous DNA but smaller than full ambiguous
            Test.@test length(Mycelia.UNAMBIGUOUS_DNA_CHARSET) <
                       length(Mycelia.ACGTN_DNA_CHARSET)
            Test.@test length(Mycelia.ACGTN_DNA_CHARSET) <
                       length(Mycelia.AMBIGUOUS_DNA_CHARSET)

            ## ACGUN should be larger than unambiguous RNA but smaller than full ambiguous
            Test.@test length(Mycelia.UNAMBIGUOUS_RNA_CHARSET) <
                       length(Mycelia.ACGUN_RNA_CHARSET)
            Test.@test length(Mycelia.ACGUN_RNA_CHARSET) <
                       length(Mycelia.AMBIGUOUS_RNA_CHARSET)

            ## Verify that AA alphabet is larger than nucleotide alphabets (more amino acids than nucleotides)
            Test.@test length(Mycelia.UNAMBIGUOUS_DNA_CHARSET) <
                       length(Mycelia.UNAMBIGUOUS_AA_CHARSET)
            Test.@test length(Mycelia.UNAMBIGUOUS_RNA_CHARSET) <
                       length(Mycelia.UNAMBIGUOUS_AA_CHARSET)

            ## Full ambiguous alphabets should be the largest
            Test.@test length(Mycelia.UNAMBIGUOUS_AA_CHARSET) <
                       length(Mycelia.AMBIGUOUS_AA_CHARSET)
