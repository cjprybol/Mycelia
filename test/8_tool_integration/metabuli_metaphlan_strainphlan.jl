...
Test.@testset "Metabuli / MetaPhlAn / StrainPhlAn integration" begin
    ...
    if !isfile(virus_ref) || filesize(virus_ref) == 0 || !isfile(bacteria_ref) ||
       filesize(bacteria_ref) == 0
    ...
    try
        ...
        Test.@testset "StrainPhlAn end-to-end" begin
            ...
            Test.@test sp_res.marker_alignment === nothing ||
                       isfile(sp_res.marker_alignment)
        end
    finally
        rm(workdir; recursive = true, force = true)
