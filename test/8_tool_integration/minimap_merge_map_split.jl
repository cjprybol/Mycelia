...
#
# ```bash
# MYCELIA_RUN_ALL=true MYCELIA_RUN_EXTERNAL=true julia --project=. -e 'include("test/8_tool_integration/minimap_merge_map_split.jl")'
# ```
#
...
Test.@testset "Minimap merge/map/split helpers" begin
    ...
    Test.@testset "Mapping generates sorted BAMs with index-aware names" begin
        ...
        if !run_external
            ...
            expected_bam = joinpath(
                dirname(fq), "$(expected_fastq_label).$(basename(res.index_file)).sorted.bam")
    ...
    Test.@testset "UUID mapping splits are recoverable (per-sample and joint)" begin
        ...
        if !run_external
            ...
            try
                ...
                isdir(joint_res.tmpdir) &&
                    rm(joint_res.tmpdir; recursive = true, force = true)
            end
                ...
                if genome_info.source != :ncbi
                    ...
                    function build_records(seq::String, positions::Vector{Int}, read_len::Int, prefix::String)
                        ...
                        for (i, pos) in enumerate(positions)
                            ...
                            push!(records,
                                FASTX.FASTQ.Record("$(prefix)_$(i)", subseq, repeat("I", read_len)))
                        end
                        return records
                    end
                    ...
                    function collect_primary_mappings(bam::String;
                            id_map::Dict{String, String} = Dict{String, String}())
                        ...
                        for row in eachrow(df)
                            ...
                            push!(tuples, (read_id, row.reference,
                                first(row.position), row.alignlength))
                        end
                        return sort(tuples)
                    end
                    ...
                    function flatten_read_map(mapping_by_source::Dict{
                    ...
                    function run_consistency_case(;
                            ref, seq, mapping_type, read_len, use_index)
                        max_start = length(seq) - read_len + 1
                        positions_a = [1, min(1 + read_len, max_start)]
                        positions_b = [
                            min(1 + 3 * read_len, max_start), min(1 + 5 * read_len, max_start)]
