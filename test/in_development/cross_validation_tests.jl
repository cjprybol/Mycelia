...
Test.@testset "Cross-Validation Tests" begin
    Test.@testset "Core Framework Functions" begin
        ...
        Test.@test hasmethod(Mycelia.create_kfold_partitions, (
            Vector{FASTX.FASTQ.Record}, Int, Float64))
        Test.@test hasmethod(Mycelia.validate_assemblies_against_holdout,
            (Dict, Dict, Vector{FASTX.FASTQ.Record}, Int, String))
        Test.@test hasmethod(
            Mycelia.calculate_assembly_mapping_metrics, (
                Any, Vector{FASTX.FASTQ.Record}, String))
        Test.@test hasmethod(Mycelia.compare_assembly_statistics, (Dict, Dict))
        Test.@test hasmethod(Mycelia.generate_consensus_pangenome, (
    ...
    Test.@testset "Assembly Mapping Metrics" begin
        ...
        validation_records = [FASTX.FASTQ.Record("val$i", seq, repeat("I", length(seq)))
