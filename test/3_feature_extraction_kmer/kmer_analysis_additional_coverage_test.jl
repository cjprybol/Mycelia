...
Test.@testset "K-mer Analysis Additional Coverage" begin
    mktempdir() do temp_dir
        ...
        Test.@testset "Result loading edge cases" begin
            ...
            malformed_metadata_path = joinpath(temp_dir, "malformed_metadata.jld2")
            JLD2.jldopen(malformed_metadata_path, "w") do file
                file["kmers"] = [Kmers.DNAKmer{3}("AAA")]
                file["counts"] = reshape(UInt16[4, 2], 1, 2)
                file["fasta_list"] = ["a.fasta", "b.fasta"]
                file["metadata/k"] = 3
            ...
            malformed_loaded = Mycelia.load_kmer_results(malformed_metadata_path)
            Test.@test malformed_loaded !== nothing
            Test.@test malformed_loaded.metadata["alphabet"] == "DNA\0broken"
        ...
        Test.@testset "Dense and sparse file count helpers" begin
            ...
            dense_serial_temp = Mycelia.fasta_list_to_dense_kmer_counts(
                fasta_list = [fasta],
                k = 3,
                alphabet = :DNA,
                force_threading = false,
            ...
            Test.@test dense_serial_temp.kmers == dense_result.kmers
            Test.@test dense_serial_temp.counts == dense_result.counts
