...
Test.@testset "Coverage Clustering" begin
    ...
    filtered, filter_stats, removed = Mycelia.automatic_error_filtering(
        kmer_counts;
        separation_threshold=0.0,
        max_error_rate=1.0
    )
    Test.@test length(filtered) + length(removed) == length(kmer_counts)
    Test.@test filter_stats.error_cluster_size + filter_stats.signal_cluster_size == length(kmer_counts)
