...
# - `test/4_assembly/third_party_assemblers_legacy.jl` (older notebook-style file)

run_external =
    isdefined(@__MODULE__, :MYCELIA_RUN_EXTERNAL) ? getfield(@__MODULE__, :MYCELIA_RUN_EXTERNAL) :
    lowercase(get(ENV, "MYCELIA_RUN_ALL", "false")) == "true" || lowercase(get(ENV, "MYCELIA_RUN_EXTERNAL", "false")) == "true"

if run_external
    include("third_party_assemblers_short_read_isolate.jl")
    include("third_party_assemblers_short_read_metagenomic.jl")
    include("third_party_assemblers_long_read_isolate.jl")
    include("third_party_assemblers_long_read_metagenomic.jl")
    include("third_party_assemblers_hybrid.jl")
    include("third_party_assemblers_plass_penguin.jl")
end
