...
Test.@testset "Long Read Isolate Assembly" begin
    mktempdir() do dir
        Test.@testset "Long Read Isolate Assembly - Flye" begin
            ...
            flye_simulated_reads = Mycelia.simulate_nanopore_reads(
                fasta = flye_ref_fasta, quantity = "20x", quiet = true, seed = 456)
            ...
            try
                result = Mycelia.run_flye(fastq = flye_fastq, outdir = flye_outdir,
                    read_type = "nano-hq", threads = threads)
        ...
        Test.@testset "Long Read Isolate Assembly - Canu" begin
            ...
            canu_simulated_reads = Mycelia.simulate_pacbio_reads(
                fasta = canu_ref_fasta, quantity = "12x", quiet = true, seed = 42)
            ...
            try
                result = Mycelia.run_canu(
                    fastq = canu_fastq, outdir = canu_outdir, genome_size = "15k",
                    stopOnLowCoverage = 8, threads = threads)
                Test.@test result.outdir == canu_outdir
                Test.@test result.assembly ==
                           joinpath(canu_outdir, "canu_reads.contigs.fasta")
                Test.@test isfile(result.assembly)
                # Clean up after test
        ...
        Test.@testset "Long Read Isolate Assembly - Hifiasm" begin
            ...
            is_ci = haskey(ENV, "CI") || haskey(ENV, "GITHUB_ACTIONS") ||
                    haskey(ENV, "TRAVIS") || haskey(ENV, "CIRCLECI")
            ...
            if is_ci
                ...
                hifiasm_simulated_reads = Mycelia.simulate_pacbio_reads(
                    fasta = hifiasm_ref_fasta, quantity = "10x", quiet = true, seed = 333)
                ...
                try
                    result = Mycelia.run_hifiasm(
                        fastq = hifiasm_fastq, outdir = hifiasm_outdir,
                        bloom_filter = 0, threads = threads)
                    Test.@test result.outdir == hifiasm_outdir
                    expected_prefix = joinpath(hifiasm_outdir, basename(hifiasm_fastq) *
                                                               ".hifiasm")
