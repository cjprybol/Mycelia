# DNA Qualmer Graph - DoubleStrand Mode Test
#
# Run with: julia --project=. -e 'include("test/4_assembly/dna_qualmer_doublestrand_test.jl")'
...
Test.@testset "DNA Qualmer DoubleStrand Graph" begin
    ...
    for vertex_label in vertices
        vertex_data = graph[vertex_label]
        strands = [strand for (obs_id, pos, strand) in vertex_data.coverage if length(cov_entry) >= 3]
