...
Test.@testset "Short Read Metagenomic Assembly" begin
    mktempdir() do dir
        ...
        meta_simulated_reads = Mycelia.simulate_illumina_reads(fasta=meta_ref_fasta, coverage=10, rndSeed=456, quiet=true)
        ...
        Test.@testset "Short Read Metagenomic Assembly - MEGAHIT" begin
            ...
            try
                result = Mycelia.run_megahit(fastq1=meta_fastq1, fastq2=meta_fastq2, outdir=megahit_outdir, k_list="21,29", threads=threads)
        ...
        Test.@testset "Short Read Metagenomic Assembly - metaSPAdes" begin
            ...
            try
                result = Mycelia.run_metaspades(fastq1=meta_fastq1, fastq2=meta_fastq2, outdir=spades_outdir, k_list="21", threads=threads, only_assembler=true)
...
Test.@testset "Long Read Metagenomic Assembly" begin
    mktempdir() do dir
        ...
        meta_long_simulated_reads = Mycelia.simulate_pacbio_reads(fasta=meta_long_ref_fasta, quantity="10x", quiet=true)
        ...
        Test.@testset "Long Read Metagenome Assembly - hifiasm-meta" begin
            ...
            try
                result = Mycelia.run_hifiasm_meta(fastq=meta_long_fastq, outdir=hifiasm_meta_outdir, bloom_filter=0, read_selection=true, threads=threads)
        ...
        Test.@testset "Long Read Metagenomic Assembly - metamdbg HiFi" begin
            ...
            try
                result = Mycelia.run_metamdbg(hifi_reads=meta_long_fastq, outdir=metamdbg_hifi_outdir, abundance_min=2, threads=threads)
        ...
        Test.@testset "Long Read Metagenomic Assembly - metamdbg ONT" begin
            ...
            try
                result = Mycelia.run_metamdbg(ont_reads=ont_fastq, outdir=metamdbg_ont_outdir, abundance_min=2, threads=threads)
