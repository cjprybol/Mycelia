...
Test.@testset "Linear Chain Collapsing (collapse_linear_chains!)" begin
    ...
    Test.@testset "Evidence Merging" begin
        Test.@testset "Type-check audit reduced vertex dispatch" begin
            reduced_vertex_cases = [
                (
                    "lightweight_string",
                    String,
                    Mycelia.Rhizomorph.LightweightStringVertexData,
                    Mycelia.Rhizomorph.LightweightEdgeData,
                    "AB",
                    "BC",
                    "ABC",
                    1,
                ...
                (
                    "ultralight_string",
                    String,
                    Mycelia.Rhizomorph.UltralightStringVertexData,
                ...
                (
                    "ultralight_quality_biosequence",
                    BioSequences.LongDNA{4},
                    Mycelia.Rhizomorph.UltralightQualityBioSequenceVertexData{BioSequences.LongDNA{4}},
                    Mycelia.Rhizomorph.UltralightQualityEdgeData,
                    BioSequences.LongDNA{4}("ATG"),
                    BioSequences.LongDNA{4}("TGC"),
                    BioSequences.LongDNA{4}("ATGC"),
                    2,
                    true
                ),
                (
                    "lightweight_quality_biosequence",
                    BioSequences.LongDNA{4},
                    Mycelia.Rhizomorph.LightweightQualityBioSequenceVertexData{BioSequences.LongDNA{4}},
            ...
            for (
                    ...
                    overlap_length,
                    has_quality
            ) in reduced_vertex_cases
                Test.@testset "$name dispatches via AllReducedVertexData" begin
                    graph = MetaGraphsNext.MetaGraph(
                        Graphs.DiGraph();
                    ...
                    first_vertex = vertex_data_type(first_label)
                    second_vertex = vertex_data_type(second_label)

                    if has_quality
                        quality_entry_a = Mycelia.Rhizomorph.QualityEvidenceEntry(
                            1,
                            Mycelia.Rhizomorph.Forward,
                            UInt8[43, 43, 43]
                        )
                        quality_entry_b = Mycelia.Rhizomorph.QualityEvidenceEntry(
                            1,
                            Mycelia.Rhizomorph.Forward,
                            UInt8[44, 44, 44]
                        )
                        Mycelia.Rhizomorph.add_evidence!(
                            first_vertex, "ds1", "obs1", quality_entry_a)
                        Mycelia.Rhizomorph.add_evidence!(
                            second_vertex, "ds1", "obs2", quality_entry_b)
                    else
                        evidence_entry = Mycelia.Rhizomorph.EvidenceEntry(
                            1, Mycelia.Rhizomorph.Forward)
                        Mycelia.Rhizomorph.add_evidence!(
                            first_vertex, "ds1", "obs1", evidence_entry)
                        Mycelia.Rhizomorph.add_evidence!(
                            second_vertex, "ds1", "obs2", evidence_entry)
                    end

                    graph[first_label] = first_vertex
                    graph[second_label] = second_vertex
                    graph[first_label, second_label] = edge_data_type(overlap_length)

                    Test.@test first_vertex isa Mycelia.Rhizomorph.AllReducedVertexData
                    Test.@test second_vertex isa Mycelia.Rhizomorph.AllReducedVertexData

                    Mycelia.Rhizomorph.collapse_linear_chains!(graph)

                    Test.@test haskey(graph, collapsed_label)
                    collapsed = graph[collapsed_label]
                    Test.@test collapsed isa vertex_data_type
                    Test.@test collapsed.total_count == 2
                    Test.@test collapsed.dataset_counts["ds1"] == 2

                    if has_quality
                        Test.@test collapsed.joint_quality == UInt8[21, 21, 21]
                    elseif hasfield(typeof(collapsed), :dataset_observations)
                        Test.@test collapsed.dataset_observations["ds1"] == Set(["obs1", "obs2"])
                    end
                end
            end
        end
