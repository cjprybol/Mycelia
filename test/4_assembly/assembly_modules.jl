...
Test.@testset "assembly modules" begin
    ...
    Test.@testset "3. Short Read Assembly" begin
        Test.@testset "3a. Short Read Isolate Assembly" begin
            mktempdir() do dir
        ...
        Test.@testset "3b. Short Read Metagenomic Assembly" begin
            mktempdir() do dir
                # Create small reference genome for metagenomic short read assembly
                meta_ref_fasta = joinpath(dir, "metagenomic_ref.fasta")
                rng_meta = StableRNGs.StableRNG(456)
                meta_genome = BioSequences.randdnaseq(rng_meta, 4000)  # 4kb genome for metagenomic simulation
                
                # Create FASTA record and write using Mycelia.write_fasta
                meta_fasta_record = FASTX.FASTA.Record("test_metagenomic_genome", meta_genome)
                Mycelia.write_fasta(outfile=meta_ref_fasta, records=[meta_fasta_record])
    ...
    Test.@testset "4. Long Read Assembly" begin
        Test.@testset "4a. Long Read Isolate Assembly" begin
            mktempdir() do dir
            ...
            # Test hifiasm - reuse the canu simulated PacBio reads (HiFi-like)
            hifiasm_outdir = joinpath(dir, "hifiasm_assembly")
            if isdir(hifiasm_outdir)
                rm(hifiasm_outdir, recursive=true)
            end
            try
                result = Mycelia.run_hifiasm(fastq=canu_fastq, outdir=hifiasm_outdir)
                Test.@test result.outdir == hifiasm_outdir
                expected_prefix = joinpath(hifiasm_outdir, basename(canu_fastq) * ".hifiasm")
        ...
        Test.@testset "4b. Long Read Metagenomic Assembly" begin
            mktempdir() do dir
                # Create reference genome for long read metagenomic assembly
                meta_long_ref_fasta = joinpath(dir, "metagenomic_long_ref.fasta")
                rng_meta_long = StableRNGs.StableRNG(789)
                meta_long_genome = BioSequences.randdnaseq(rng_meta_long, 8000)  # 8kb genome for metagenomic simulation
                
                # Create FASTA record and write using Mycelia.write_fasta
                meta_long_fasta_record = FASTX.FASTA.Record("test_metagenomic_long_genome", meta_long_genome)
                Mycelia.write_fasta(outfile=meta_long_ref_fasta, records=[meta_long_fasta_record])
            ...
            # Test metamdbg - clean up any existing directory first
            metamdbg_outdir = joinpath(dir, "metamdbg_assembly")
            if isdir(metamdbg_outdir)
                rm(metamdbg_outdir, recursive=true)
            end
            try
                result = Mycelia.run_metamdbg(meta_long_fastq, outdir=metamdbg_outdir, abundance_min=2, length_min=500)
                Test.@test result.outdir == metamdbg_outdir
                Test.@test result.contigs == joinpath(metamdbg_outdir, "contigs.fasta")
                Test.@test result.graph == joinpath(metamdbg_outdir, "graph.gfa")
                Test.@test isfile(result.contigs)
                # Clean up after test
                rm(metamdbg_outdir, recursive=true, force=true)
            catch e
                if isa(e, ProcessFailedException) || contains(string(e), "memory") || contains(string(e), "Memory") || contains(string(e), "killed")
                    @warn """
                    metamdbg assembly failed due to resource constraints.
                    Current test: 8kb genome, 12x coverage, ~96kb total sequence data
                    ...
                    To fix: Increase available memory or reduce test genome size further.
                    """
                    Test.@test_skip "metamdbg test skipped - insufficient resources"
                else
                    rethrow(e)
                end
                # Clean up on failure
