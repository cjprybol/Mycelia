...
Test.@testset "RNA K-mer SingleStrand Graph (Rhizomorph)" begin
    ...
    for (vertex_label, expected_positions) in Dict(
        ...
        Test.@test Mycelia.Rhizomorph.count_evidence(vertex_data) ==
        ...
        observation_evidence = Mycelia.Rhizomorph.get_observation_evidence(
            vertex_data, dataset_id, observation_id)
        ...
        Test.@test all(entry.strand == Mycelia.Rhizomorph.Forward
    ...
    for edge_label in MetaGraphsNext.edge_labels(graph)
        ...
        observation_evidence = Mycelia.Rhizomorph.get_observation_evidence(
            edge_data, dataset_id, observation_id)
        Test.@test observation_evidence !== nothing
        pairs = sort(collect((entry.from_position, entry.to_position)
        for entry in observation_evidence))
    ...
    walk_steps = [Mycelia.Rhizomorph.WalkStep(label, Mycelia.Rhizomorph.Forward, 1.0, Float64(i))
