# Test for the degree-of-repetition x biological-repeat-class prime(coprime) vs
# composite(factor-sharing) k ablation (bead td-tjym).
#
# Encodes the HONEST, POWERED result. It asserts:
#   (1) the PRIMARY resolution metric (largest correct contig / reference) has
#       DYNAMIC RANGE and does NOT saturate on the hard tandem fixtures — so any
#       null is a powered negative, not a saturation artifact (the v3 flaw);
#   (2) for contrast, the w-mer-recovery metric DOES saturate above the k-size
#       threshold (documenting why it is blind to repeat resolution);
#   (3) the size-controlled ISOLATING COMPARISON — no factor-sharing k
#       (gcd(k,period)>1) robustly-strikingly dips below the coprime-k size
#       envelope (interpolated between coprime k below & above), across the swept
#       degree of repetition and biological repeat classes; and
#   (4) the HARNESS-SENSITIVITY collision control fires — proving the metric CAN
#       detect an aliasing deficit, so the factor-sharing null is real.
...
Test.@testset "rhizomorph prime(coprime)-vs-composite(factor-sharing) degree+class ablation" begin
    ...
    Test.@testset "no isolated factor-sharing deficit (size-controlled, powered null)" begin
        ...
        # No factor-sharing k robustly-strikingly dips below the coprime envelope.
