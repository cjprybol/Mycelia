    ...
    mktempdir() do dir
        Test.@testset "Flye" begin
            ...
            Mycelia.write_fasta(outfile = ref_fasta,
                records = [FASTX.FASTA.Record("flye_isolate_genome", genome)])
        ...
        Test.@testset "Hifiasm" begin
            ...
            Mycelia.write_fasta(outfile = ref_fasta,
                records = [FASTX.FASTA.Record("hifiasm_test_genome", genome)])
            ...
            try
                ...
                Test.@test result.outdir == outdir
                Test.@test result.hifiasm_outprefix ==
                           joinpath(outdir, basename(reads_fastq) * ".hifiasm")
            finally
                rm(outdir, recursive = true, force = true)
            end
        end
