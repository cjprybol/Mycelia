...
#
# ```bash
# julia --project=. -e 'include("test/1_data_acquisition/simulation_fastq.jl")'
# ```
#
...
Test.@testset "FASTQ simulation" begin
    ...
    if !isfile(test_fasta) || filesize(test_fasta) == 0
        ...
        Test.@testset "Illumina" begin
            ...
            read_simulation_result = Mycelia.simulate_illumina_reads(
                fasta = test_fasta, coverage = 10, quiet = true, errfree = true)
        ...
        Test.@testset "Ultima Genomics" begin
            ...
            read_simulation_result = Mycelia.simulate_ultima_reads(
                fasta = test_fasta, coverage = 5, quiet = true)
            Test.@test isfile(read_simulation_result.forward_reads)
        ...
        Test.@testset "Nanopore" begin
            read_simulation_result = Mycelia.simulate_nanopore_reads(
                fasta = test_fasta, quantity = 10, quiet = true)
            Test.@test isfile(read_simulation_result)
            Test.@test endswith(read_simulation_result, ".fq.gz")
        end

        Test.@testset "PacBio" begin
            read_simulation_result = Mycelia.simulate_pacbio_reads(
                fasta = test_fasta, quantity = 10, quiet = true)
            Test.@test isfile(read_simulation_result)
            Test.@test endswith(read_simulation_result, ".fq.gz")
        end
        ...
        Test.@testset "Nearly Perfect Long Reads" begin
            read_simulation_result = Mycelia.simulate_nearly_perfect_long_reads(
                fasta = test_fasta, quantity = "10x", quiet = true)
            Test.@test isfile(read_simulation_result)
            Test.@test endswith(read_simulation_result, ".fq.gz")
        end

        Test.@testset "Nanopore R9.4.1 Reads" begin
            read_simulation_result = Mycelia.simulate_nanopore_r941_reads(
                fasta = test_fasta, quantity = "5x", quiet = true)
            Test.@test isfile(read_simulation_result)
            Test.@test endswith(read_simulation_result, ".fq.gz")
        ...
        Test.@testset "Very Bad Reads" begin
            read_simulation_result = Mycelia.simulate_very_bad_reads(
                fasta = test_fasta, quantity = "5x", quiet = true)
            Test.@test isfile(read_simulation_result)
            Test.@test endswith(read_simulation_result, ".fq.gz")
        ...
        Test.@testset "Pretty Good Reads" begin
            read_simulation_result = Mycelia.simulate_pretty_good_reads(
                fasta = test_fasta, quantity = "5x", quiet = true)
            Test.@test isfile(read_simulation_result)
            Test.@test endswith(read_simulation_result, ".fq.gz")
        ...
        Test.@testset "Error Model Coverage Tests" begin
            ...
            error_models = ["nanopore2018", "nanopore2020", "nanopore2023",
                "pacbio2016", "pacbio2021", "random"]
            ...
            qscore_models = ["nanopore2018", "nanopore2020", "nanopore2023",
                "pacbio2016", "pacbio2021", "random", "ideal"]
