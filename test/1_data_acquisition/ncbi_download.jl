...
Test.@testset "ncbi_genome_download_accession_partial" begin
    ...
    try
        result = Mycelia.ncbi_genome_download_accession(
            accession = accession, include_string = "genome", outdir = workdir)
        ...
        result2 = Mycelia.ncbi_genome_download_accession(
            accession = accession, include_string = "protein", outdir = workdir)
...
Test.@testset "SRA prefetch and fasterq_dump stepwise" begin
    ...
    sra_public_fastq_metadata = CSV.read(
        CodecZlib.GzipDecompressorStream(open(sra_public_fastq_metadata_file)), DataFrames.DataFrame)
    ...
    try
        ...
        Test.@test isfile(fasterq_result.unpaired_reads) &&
                   filesize(fasterq_result.unpaired_reads) > 0
    finally
...
Test.@testset "SRA downloading" begin
    ...
    try
        Test.@testset "download SRA short read data" begin
            ...
            Test.@test prefetch_results.archive ==
                       joinpath(outdir, srr_identifier, "$(srr_identifier).sra")
            ...
            Test.@test fasterq_dump_result.forward_reads ==
                       joinpath(outdir, srr_identifier, "$(srr_identifier)_1.fastq.gz")
            Test.@test fasterq_dump_result.reverse_reads ==
                       joinpath(outdir, srr_identifier, "$(srr_identifier)_2.fastq.gz")
        ...
        Test.@testset "download SRA long read data" begin
            ...
            Test.@test fasterq_dump_result.unpaired_reads ==
                       joinpath(outdir, srr_identifier, "$(srr_identifier).fastq.gz")
