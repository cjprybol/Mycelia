...
| Assembly validation | `compute_merqury_qv`, `assess_assembly_quality` | `src/quality-control-and-benchmarking.jl` | native QV checks via `test/5_validation/validation.jl` | Native Merqury-style QV exists; external Merqury CLI wrapper and spectra/completeness parsers are planned in `planning-docs/BENCHMARK_EVALUATION_WRAPPER_PLAN.md`. |
| Assembly validation | Inspector | planned | none | No active wrapper yet; interface, applicability boundary, dependency behavior, and output schema are planned in `planning-docs/BENCHMARK_EVALUATION_WRAPPER_PLAN.md`. |
