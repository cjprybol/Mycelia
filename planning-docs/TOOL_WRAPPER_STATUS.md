...
| Annotation gap | `run_egapx` | `src/annotation.jl` | validation only | Intentional placeholder wrapper: `run_egapx` always throws `ArgumentError` documenting that EGAPx is not currently supported in Mycelia; see tracking bead `my-a14` / source `td-ex8mp`. |
