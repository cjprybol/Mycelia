    ...
    try
        ...
        for key in keys(loaded_data)
            if startswith(key, "metadata/") &&
               !haskey(metadata, replace(key, "metadata/" => ""))
...
"""
function jellyfish_count(; fastx, k, threads = get_default_threads(),
        max_mem = Int(Sys.free_memory()), canonical = false,
        outfile = ifelse(canonical, "$(fastx).k$(k).canonical.jf", "$(fastx).k$(k).jf"),
        conda_check = true)
    if conda_check
    ...
    jellyfish_mem_output = read(
        `$(Mycelia.CONDA_RUNNER) run --live-stream -n kmer-jellyfish jellyfish mem --mer-len $(k) --mem $(mem)`,
        String)
...
"""
function jellyfish_counts_to_kmer_frequency_histogram(jellyfish_counts_file,
        outfile = replace(jellyfish_counts_file, r"\.tsv\.gz$" => ".count_histogram.tsv"))
    # sorting with LC_ALL=C is the biggest speed up here of anything I've found
    if !isfile(outfile)
        io = open(pipeline(
            `gzip -dc $(jellyfish_counts_file)`,
            `cut -f2`,
            Cmd(`sort --temporary-directory . --compress-program gzip --numeric --stable`,
                env = Dict("LC_ALL" => "C")),
            `uniq --count`,
            `sed 's/^ *//'`,
            `sed 's/ /\t/'`
        ))
        frequency_histogram_table = CSV.read(io, DataFrames.DataFrame,
            header = ["number of kmers", "number of observations"], delim = '\t')
...
function load_jellyfish_counts(jellyfish_counts)
    ...
    open(jellyfish_counts) do io
        table = CSV.read(CodecZlib.GzipDecompressorStream(io), DataFrames.DataFrame,
            delim = "\t", header = ["kmer", "count"])
...
function analyze_kmer_spectra(; out_directory, forward_reads = "", reverse_reads = "",
        k = 17, target_coverage = 0, plot_size = (600, 400))
...
function strobemers(sequence, k::Int, w_min::Int, w_max::Int; canonical::Bool = false)
    ...
    for i in 1:length(kmers)
        ...
        if j_start <= j_end
            ...
            first = canonical && _is_nucleotide_sequence(sequence) ?
                    BioSequences.canonical(kmers[i]) : kmers[i]
...
"""
function assess_dnamer_saturation(fastxs::AbstractVector{<:AbstractString}, kmer_type;
        kmers_to_assess = Inf, power = 10, min_count = 1)
        ...
        for record in open_fastx(fastx)
            ...
            for (index, kmer) in Kmers.EveryKmer{kmer_type}(record_sequence)
                ...
                if (length(canonical_kmer_counts) == max_possible_kmers)
                    ...
                    unique_kmer_counts = vcat(
                        unique_kmer_counts[1:(length(sampling_points) - 1)], length(canonical_kmer_counts))
                ...
                elseif kmers_assessed in sampling_points
                    ...
                    if i == length(sampling_points)
                        return (sampling_points = sampling_points,
                            unique_kmer_counts = unique_kmer_counts, eof = false)
                    end
                end
    ...
    unique_kmer_counts = vcat(
        unique_kmer_counts[1:(length(sampling_points) - 1)], [length(canonical_kmer_counts)])
    return (sampling_points = sampling_points,
        unique_kmer_counts = unique_kmer_counts, eof = true)
end
...
"""
function assess_dnamer_saturation(fastxs::AbstractVector{<:AbstractString}; power = 10,
        outdir::Union{Missing, String} = missing, min_k = 7, max_k = 17,
        threshold = 0.1, kmers_to_assess = 10_000_000, plot = true)
    ...
    for k in ks
        ...
        observed_midpoint_index = findfirst(
            i -> kmer_counts[i] > last(kmer_counts)/2, 1:length(sampling_points))
        ...
        if plot
            ...
            p = StatsPlots.scatter(
                ...
                ytickfontsize = fontsize-4                # xrotation=45
            )
...
"""
function assess_dnamer_saturation(
        fastx::AbstractString; power = 10, outdir = "", min_k = 3,
        max_k = 17, threshold = 0.1, kmers_to_assess = 10_000_000)
    assess_dnamer_saturation([fastx], outdir = outdir, min_k = min_k, max_k = max_k,
        threshold = threshold, power = power, kmers_to_assess = kmers_to_assess)
end
...
function fasta_list_to_sparse_kmer_counts(;
    ...
    function _get_output_dir()
        if !isnothing(out_dir)
            return out_dir
        elseif !isnothing(result_file) && !isempty(Base.Filesystem.dirname(result_file)) &&
               Base.Filesystem.dirname(result_file) != "."
    ...
    function _resolve_outpath(fname::AbstractString)
        (Base.Filesystem.isabspath(fname) || isempty(fname)) ? fname :
        Base.Filesystem.joinpath(output_dir, fname)
    end
    ...
    KMER_TYPE = if alphabet == :AA
        isdefined(Kmers, :AAKmer) ? Kmers.AAKmer{k} :
        error("Kmers.AAKmer not found or Kmers.jl not loaded correctly.")
    elseif alphabet == :DNA
        isdefined(Kmers, :DNAKmer) ? Kmers.DNAKmer{k} :
        error("Kmers.DNAKmer not found or Kmers.jl not loaded correctly.")
    elseif alphabet == :RNA
        isdefined(Kmers, :RNAKmer) ? Kmers.RNAKmer{k} :
        error("Kmers.RNAKmer not found or Kmers.jl not loaded correctly.")
    else
    ...
    COUNT_FUNCTION = if alphabet == :DNA
        ...
        isdefined(Main, func_name) ? getfield(Main, func_name) :
        (isdefined(@__MODULE__, func_name) ? getfield(@__MODULE__, func_name) :
         error("$func_name not found"))
    else
    ...
    use_threading = if !isnothing(force_threading)
        ...
        (num_files >= threading_file_threshold &&
         total_file_size > threading_size_threshold) ||
    ...
    compute_rarefaction = !skip_rarefaction_data &&
                          (show_progress || !skip_rarefaction_plot)
    ...
    temp_file_paths = [Base.Filesystem.joinpath(temp_dir, "counts_$(i).jld2")
    ...
    progress_pass1 = ProgressMeter.Progress(
        num_files; desc = "Pass 1 (Counting & Writing Temps): ",
        barglyphs = ProgressMeter.BarGlyphs("[=> ]"), color = :cyan)
    ...
    progress_pass2 = show_progress ?
                     ProgressMeter.Progress(
        num_files; desc = "Pass 2 (Aggregating Serially): ",
        barglyphs = ProgressMeter.BarGlyphs("[=> ]"), color = :yellow) : nothing
    ...
    for i in 1:num_files
        ...
        if show_progress && !isnothing(progress_pass2)
            if compute_rarefaction
                ProgressMeter.next!(progress_pass2; showvalues = [(
                    :unique_kmers, length(all_kmers_set))])
            else
    ...
    if isempty(all_kmers_set) && total_non_zero_entries == 0
        ...
        result = (; kmers = Vector{KMER_TYPE}(),
            counts = SparseArrays.spzeros(ValType, Int, 0, num_files),
            rarefaction_data_path = rarefaction_data_path)
    ...
    empty!(all_kmers_set);
    all_kmers_set = nothing;
    GC.gc()

    # Estimate memory needed for sparse matrix
    sparse_matrix_bytes_needed = Mycelia.estimate_sparse_matrix_memory(
        ValType, num_kmers, num_files, nnz = total_non_zero_entries)
    ...
    kmer_to_row_map = Dict{KMER_TYPE, Int}(kmer => i
    ...
    progress_pass3 = ProgressMeter.Progress(
        num_files; desc = "Pass 3 (Filling Sparse Data): ",
        barglyphs = ProgressMeter.BarGlyphs("[=> ]"), color = :blue)
    ...
    final_result = (; kmers = sorted_kmers, counts = kmer_counts_sparse_matrix,
        rarefaction_data_path = rarefaction_data_path)
...
- `K`: Length of k-mers
"""
function count_kmers(::Type{Kmers.Kmer{A, K}},
        sequence::BioSequences.LongSequence) where {A <: BioSequences.DNAAlphabet, K}
    # return sort(StatsBase.countmap(Kmers.FwDNAMers{K}(sequence)))
    kmer_counts = StatsBase.countmap([kmer
                                      for (kmer, index) in
                                          Kmers.UnambiguousDNAMers{K}(sequence)])
...
"""
function count_kmers(::Type{Kmers.Kmer{A, K}},
        sequence::BioSequences.LongSequence) where {A <: BioSequences.RNAAlphabet, K}
    # return sort(StatsBase.countmap(Kmers.FwRNAMers{K}(sequence)))
    kmer_counts = StatsBase.countmap([kmer
                                      for (kmer, index) in
                                          Kmers.UnambiguousRNAMers{K}(sequence)])
    return sort!(OrderedCollections.OrderedDict(kmer_counts))
end
...
"""
function count_kmers(::Type{Kmers.Kmer{A, K}},
        sequence::BioSequences.LongSequence) where {A <: BioSequences.AminoAcidAlphabet, K}
...
A sorted dictionary mapping each k-mer to its frequency count in the sequence.
"""
function count_kmers(::Type{KMER_TYPE},
        record::R) where {KMER_TYPE, R <: Union{FASTX.FASTA.Record, FASTX.FASTQ.Record}}
...
"""
function count_kmers(::Type{KMER_TYPE},
        records::AbstractVector{T}) where {
        KMER_TYPE, T <: Union{FASTX.FASTA.Record, FASTX.FASTQ.Record}}
...
"""
function count_kmers(::Type{KMER_TYPE},
        sequences::R) where {KMER_TYPE, R <: Union{FASTX.FASTA.Reader, FASTX.FASTQ.Reader}}
...
"""
function count_kmers(::Type{KMER_TYPE}, fastx_files::AbstractVector{T}) where {
        KMER_TYPE, T <: AbstractString}
...
function _dense_kmer_counts_in_memory(
    ...
    progress = show_progress ?
               ProgressMeter.Progress(num_files; desc = "Counting (in-memory): ", color = :cyan) :
               nothing
    ...
    progress2 = show_progress ?
                ProgressMeter.Progress(num_files; desc = "Building matrix: ", color = :green) :
                nothing
...
function fasta_list_to_dense_kmer_counts(;
    ...
    function process_single_file(idx)
        ...
        try
            ...
            try
                JLD2.save_object(temp_file, Dict{KMER_TYPE, Int}())
            catch
            end
        end
        if show_progress
    ...
    sparse_matrix_bytes_needed = Mycelia.estimate_sparse_matrix_memory(
        ValType, num_kmers, num_successful_files, nnz = count(kmer_counts_matrix .!= 0))
    ...
    result = (; kmers = sorted_kmers, counts = kmer_counts_matrix,
        successful_fasta_list = successful_fasta_list, error_log = error_log)
...
```
"""
function estimate_genome_size_from_kmers(
        sequence::Union{
            BioSequences.LongSequence, AbstractString}, k::Integer)
...
"""
function estimate_genome_size_from_kmers(records::AbstractVector{T},
        k::Integer) where {T <: Union{FASTX.FASTA.Record, FASTX.FASTQ.Record}}
...
"""
function multi_scale_kmer_analysis(sequences::Vector{BioSequences.LongDNA{4}};
        prime_ks::Vector{Int} = [3, 5, 7, 11, 13, 17, 19],
        window_size::Int = 50, step_size::Int = 10)
...
function adaptive_kmer_selection(multi_k_results; target_coverage::Float64 = 10.0)
    ...
    for (k, coverage) in
        sort(collect(coverage_estimates), by = x->abs(x[2] - target_coverage))
