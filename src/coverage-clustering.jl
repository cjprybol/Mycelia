...
function k_medoids_coverage_clustering(kmer_counts::Dict{T, Int}, k::Int = 2;
        ...
        for cluster_id in 1:k
            ...
            for candidate in cluster_points
                ...
                for point in cluster_points
                    total_cost += _calculate_distance(
                        features[candidate, :], features[point, :], distance_metric)
                end
    ...
    for cluster_id in 1:k
        ...
        coverage_thresholds[cluster_id] = isempty(cluster_coverages) ? 0.0 :
                                          Statistics.mean(cluster_coverages)
    end
...
"""
function analyze_coverage_clustering(result::KMedoidsCoverageResult, kmer_counts::Dict{
        T, Int}) where {T}
    coverages = collect(values(kmer_counts))
...
function _calculate_silhouettes(features::Matrix{Float64}, assignments::Vector{Int},
    ...
    for i in 1:n
        ...
        a_i = isempty(same_cluster_distances) ? 0.0 :
              Statistics.mean(same_cluster_distances)
