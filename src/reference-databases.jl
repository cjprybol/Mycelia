...
# ============================================================================
# Viroid Reference Data Acquisition
# ============================================================================
...
```
"""
function search_viroid_sequences(query::String="viroid"; 
                                taxon::String="viruses[organism]",
                                database::String="nuccore", 
                                max_results::Int=100)
    
    # Construct the search query
    full_query = "($query) AND ($taxon)"
    
    @info "Searching NCBI $database for: $full_query"
    ...
    # Add rate limiting to comply with NCBI guidelines
    sleep(0.34)  # Max 3 requests per second
    
    try
        response = HTTP.get(search_url, query=params)
        xml_content = String(response.body)
        
        # Parse XML to extract accession IDs
        # Simple regex-based parsing for robustness
        id_matches = eachmatch(r"<Id>(\d+)</Id>", xml_content)
        ids = [match.captures[1] for match in id_matches]
        
        if isempty(ids)
            @warn "No sequences found for query: $full_query"
            return String[]
        end
        
        # Convert IDs to accessions using efetch
        accessions = ids_to_accessions(ids, database)
        
        @info "Found $(length(accessions)) viroid sequences"
        return accessions
        
    catch e
        @error "Failed to search NCBI: $e"
        return String[]
    end
end
...
# Returns
- `NamedTuple`: Paths to downloaded files (genome_files, cds_files, protein_files)
...
```
"""
function download_viroid_reference_data(viroid_name::String; 
                                      outdir::String=pwd(),
                                      download_genome::Bool=true,
                                      download_cds::Bool=true, 
    ...
    genome_files = String[]
    cds_files = String[]
    protein_files = String[]
    
    @info "Downloading viroid reference data for: $viroid_name"
    
    # Download genome sequences
    if download_genome
        @info "Searching for genomic sequences..."
        genome_accessions = search_viroid_sequences(viroid_name, 
                                                   taxon="viruses[organism]",
                                                   database="nuccore", 
                                                   max_results=max_sequences)
        
        for accession in genome_accessions
            try
                genome_file = download_genome_by_accession(accession=accession, 
                                                         outdir=outdir, 
                                                         compressed=true)
                if isfile(genome_file)
                    push!(genome_files, genome_file)
                    @info "Downloaded genome: $accession -> $genome_file"
                end
            catch e
                @warn "Failed to download genome $accession: $e"
            end
        end
    end
    ...
    if download_cds
        @info "Searching for CDS sequences..."
        # Search for CDS specifically
        cds_query = "$viroid_name AND CDS[Feature Key]"
        cds_accessions = search_viroid_sequences(cds_query,
                                                taxon="viruses[organism]", 
                                                database="nuccore",
                                                max_results=max_sequences)
        
        for accession in cds_accessions
            try
                cds_file = download_sequence_by_accession(accession=accession,
                                                        outdir=outdir,
                                                        database="nuccore",
                                                        format="fasta_cds_na",
                                                        suffix="_cds.fna")
                if isfile(cds_file)
                    push!(cds_files, cds_file)
                    @info "Downloaded CDS: $accession -> $cds_file"
                end
            catch e
                @warn "Failed to download CDS $accession: $e"
            end
        end
    end
    ...
    if download_protein
        @info "Searching for protein sequences..."
        protein_accessions = search_viroid_sequences(viroid_name,
                                                    taxon="viruses[organism]",
                                                    database="protein", 
                                                    max_results=max_sequences)
        ...
        for accession in protein_accessions
            try
                protein_file = download_sequence_by_accession(accession=accession,
                                                            outdir=outdir, 
                                                            database="protein",
                                                            format="fasta",
                                                            suffix="_protein.faa")
                if isfile(protein_file)
                    push!(protein_files, protein_file)
                    @info "Downloaded protein: $accession -> $protein_file"
                end
            catch e
                @warn "Failed to download protein $accession: $e"
            end
        end
    end
    
    # Summary
    @info "Download complete: $(length(genome_files)) genome, $(length(cds_files)) CDS, $(length(protein_files)) protein files"
...
```
"""
function get_viroid_species_list()
    return [
...
```
"""
function batch_download_viroid_references(species_list::Vector{String};
                                        base_outdir::String="viroid_references/",
                                        download_genome::Bool=true,
                                        download_cds::Bool=true, 
                                        download_protein::Bool=true,
    ...
    @info "Starting batch download for $(length(species_list)) viroid species"
    
    for (i, species) in enumerate(species_list)
        @info "Processing species $i/$(length(species_list)): $species"
        
        # Create species-specific directory
        species_dir = joinpath(base_outdir, replace(species, " " => "_"))
        mkpath(species_dir)
        
        try
            # Download reference data for this species
            result = download_viroid_reference_data(species;
                                                  outdir=species_dir,
                                                  download_genome=download_genome,
                                                  download_cds=download_cds,
                                                  download_protein=download_protein, 
                                                  max_sequences=max_per_species)
            
            results[species] = result
            
            # Add summary statistics
            total_files = length(result.genome_files) + length(result.cds_files) + length(result.protein_files)
            @info "Completed $species: $total_files files downloaded"
            
        catch e
            @error "Failed to download data for $species: $e"
            results[species] = (error = string(e))
        end
        
        # Rate limiting between species
        if i < length(species_list)
            sleep(1.0)  # Respectful delay between species
        end
    end
    
    # Generate summary report
    total_genomes = sum(length(get(r, :genome_files, [])) for r in values(results) if haskey(r, :genome_files))
    total_cds = sum(length(get(r, :cds_files, [])) for r in values(results) if haskey(r, :cds_files))  
    total_proteins = sum(length(get(r, :protein_files, [])) for r in values(results) if haskey(r, :protein_files))
    
    @info "Batch download complete!"
    @info "Total files downloaded: $total_genomes genomes, $total_cds CDS, $total_proteins proteins"
    @info "Output directory: $base_outdir"
    
    return results
end

# ============================================================================
# Comprehensive Viroid Assembly Workflow
# ============================================================================

"""
$(DocStringExtensions.TYPEDSIGNATURES)

Complete viroid assembly workflow that downloads reference data and performs 
quality-aware assembly using Rhizomorph algorithms.
...
- `outdir::String`: Output directory for all results (default: "viroid_assembly_workflow/")
...
"""
function viroid_assembly_workflow(viroid_name::String;
                                outdir::String="viroid_assembly_workflow/",
                                k::Int=21,
                                simulate_coverage::Int=10, 
                                read_length::Int=150,
                                error_rate::Float64=0.01,
                                download_references::Bool=true,
                                run_assembly::Bool=true)
    
    mkpath(outdir)
    workflow_results = Dict{String, Any}()
    
    @info "=== Viroid Assembly Workflow Started ==="
    @info "Viroid: $viroid_name"
    @info "Output directory: $outdir"
    @info "Parameters: k=$k, coverage=$simulate_coverage, read_length=$read_length, error_rate=$error_rate"
    
    # Step 1: Download Reference Data
    if download_references
        @info "Step 1: Downloading viroid reference data from NCBI..."
        reference_data = download_viroid_reference_data(viroid_name;
                                                       outdir=joinpath(outdir, "references"),
                                                       download_genome=true,
                                                       download_cds=true,
                                                       download_protein=true,
                                                       max_sequences=5)
        workflow_results["reference_data"] = reference_data
        @info "Reference data acquired: $(length(reference_data.genome_files)) genome, $(length(reference_data.cds_files)) CDS, $(length(reference_data.protein_files)) protein files"
    else
        @info "Step 1: Skipping reference data download (download_references=false)"
        workflow_results["reference_data"] = nothing
    end
    
    # Step 2: Process Reference Files and Generate Observations
    if run_assembly && haskey(workflow_results, "reference_data") && workflow_results["reference_data"] !== nothing
        ref_data = workflow_results["reference_data"]
        
        @info "Step 2: Processing reference files and generating simulated observations..."
        
        # Process each reference file type
        dna_observations = []
        rna_observations = []
        protein_observations = []
        
        # Process genome files (DNA)
        for genome_file in ref_data.genome_files
            try
                @info "Processing genome file: $genome_file"
                # Read compressed FASTA file
                if endswith(genome_file, ".gz")
                    fasta_reader = FASTX.FASTA.Reader(CodecZlib.GzipDecompressorStream(open(genome_file)))
                else
                    fasta_reader = FASTX.FASTA.Reader(open(genome_file))
                end
                
                for (i, record) in enumerate(fasta_reader)
                    if i > 3  # Limit to avoid too many sequences
                        break
                    end
                    ...
                    # Generate simulated FASTQ reads
                    simulated_reads = _simulate_fastq_reads_from_sequence(sequence, identifier,
                                                                        coverage=simulate_coverage,
                                                                        read_length=read_length, 
                                                                        error_rate=error_rate,
                                                                        sequence_type="DNA")
                    
                    # Add to DNA observations
                    for (j, read) in enumerate(simulated_reads)
                        push!(dna_observations, (read, j))
                    end
                end
                close(fasta_reader)
            catch e
                @warn "Failed to process genome file $genome_file: $e"
            end
        end
        
        # Process CDS files (RNA transcripts)
        for cds_file in ref_data.cds_files
            try
                @info "Processing CDS file: $cds_file"
                if endswith(cds_file, ".gz")
                    fasta_reader = FASTX.FASTA.Reader(CodecZlib.GzipDecompressorStream(open(cds_file)))
                else
                    fasta_reader = FASTX.FASTA.Reader(open(cds_file))
                end
                ...
                for (i, record) in enumerate(fasta_reader)
                    ...
                    # Convert DNA to RNA and generate simulated reads
                    rna_sequence = replace(string(sequence), 'T' => 'U')
                    simulated_reads = _simulate_fastq_reads_from_sequence(rna_sequence, identifier * "_RNA",
                                                                        coverage=simulate_coverage,
                                                                        read_length=read_length,
                                                                        error_rate=error_rate,
                                                                        sequence_type="RNA")
                    ...
                    for (j, read) in enumerate(simulated_reads)
                        push!(rna_observations, (read, j))
                    end
                end
                close(fasta_reader)
            catch e
                @warn "Failed to process CDS file $cds_file: $e"
            end
        end
        
        # Process protein files (amino acids)  
        for protein_file in ref_data.protein_files
            try
                @info "Processing protein file: $protein_file"
                if endswith(protein_file, ".gz")
                    fasta_reader = FASTX.FASTA.Reader(CodecZlib.GzipDecompressorStream(open(protein_file)))
                else
                    fasta_reader = FASTX.FASTA.Reader(open(protein_file))
                end
                ...
                for (i, record) in enumerate(fasta_reader)
                    ...
                    # Generate simulated amino acid reads
                    simulated_reads = _simulate_fastq_reads_from_sequence(sequence, identifier * "_AA",
                                                                        coverage=simulate_coverage,
                                                                        read_length=min(read_length÷3, length(sequence)),
                                                                        error_rate=error_rate,
                                                                        sequence_type="AA")
                    
                    for (j, read) in enumerate(simulated_reads)
                        push!(protein_observations, (read, j))
                    end
                end
                close(fasta_reader)
            catch e
                @warn "Failed to process protein file $protein_file: $e"
            end
        end
        
        workflow_results["observations"] = (
            dna = dna_observations,
            rna = rna_observations, 
            protein = protein_observations
        )
        
        @info "Generated observations: $(length(dna_observations)) DNA, $(length(rna_observations)) RNA, $(length(protein_observations)) protein reads"
        
        # Step 3: Perform Quality-Aware Rhizomorph Assembly
        @info "Step 3: Performing Qualmer assembly with quality propagation..."
        
        assembly_results = Dict{String, Any}()
        
        # DNA Assembly using Qualmer algorithms
        if !isempty(dna_observations)
            @info "Assembling DNA sequences using Qualmer graph..."
            dna_config = AssemblyConfig(k=k, use_quality_scores=true,
                                       bubble_resolution=true, repeat_resolution=true)
            
            dna_result = _assemble_qualmer_graph(dna_observations, dna_config)
            assembly_results["dna"] = dna_result
            
            # Save FASTQ contigs
            if !isempty(dna_result.fastq_contigs)
                dna_fastq_file = joinpath(outdir, "dna_contigs_qualmer.fastq")
                open(dna_fastq_file, "w") do io
                    writer = FASTX.FASTQ.Writer(io)
                    for record in dna_result.fastq_contigs
                        write(writer, record)
                    end
                    close(writer)
                end
                @info "DNA FASTQ contigs saved: $dna_fastq_file"
            end
        end
        
        # RNA Assembly using Qualmer algorithms  
        if !isempty(rna_observations)
            @info "Assembling RNA sequences using Qualmer graph..."
            rna_config = AssemblyConfig(k=k, use_quality_scores=true,
                                       bubble_resolution=true, repeat_resolution=true)
            
            rna_result = _assemble_qualmer_graph(rna_observations, rna_config)
            assembly_results["rna"] = rna_result
            
            # Save FASTQ contigs
            if !isempty(rna_result.fastq_contigs)
                rna_fastq_file = joinpath(outdir, "rna_contigs_qualmer.fastq")
                open(rna_fastq_file, "w") do io
                    writer = FASTX.FASTQ.Writer(io)
                    for record in rna_result.fastq_contigs
                        write(writer, record)
                    end
                    close(writer)
                end
                @info "RNA FASTQ contigs saved: $rna_fastq_file"
            end
        end
        
        # Protein Assembly using Qualmer algorithms
        if !isempty(protein_observations)
            @info "Assembling protein sequences using Qualmer graph..."
            protein_config = AssemblyConfig(k=min(k, 7), use_quality_scores=true,  # Smaller k for proteins
                                          bubble_resolution=true, repeat_resolution=true)
            
            protein_result = _assemble_qualmer_graph(protein_observations, protein_config)
            assembly_results["protein"] = protein_result
            
            # Save FASTQ contigs (amino acid sequences with quality)
            if !isempty(protein_result.fastq_contigs)
                protein_fastq_file = joinpath(outdir, "protein_contigs_qualmer.fastq")
                open(protein_fastq_file, "w") do io
                    writer = FASTX.FASTQ.Writer(io)
                    for record in protein_result.fastq_contigs
                        write(writer, record)
                    end
                    close(writer)
                end
                @info "Protein FASTQ contigs saved: $protein_fastq_file"
            end
        end
        
        workflow_results["assembly_results"] = assembly_results
        
        # Step 4: Generate Summary Report
        @info "Step 4: Generating workflow summary report..."
        
        summary_file = joinpath(outdir, "viroid_assembly_workflow_summary.txt")
        open(summary_file, "w") do io
            println(io, "=== Viroid Assembly Workflow Summary ===")
            println(io, "Viroid: $viroid_name")
            println(io, "Date: $(Dates.now())")
            println(io, "Parameters: k=$k, coverage=$simulate_coverage, read_length=$read_length, error_rate=$error_rate")
            println(io, "")
            
            if haskey(workflow_results, "reference_data") && workflow_results["reference_data"] !== nothing
                ref_data = workflow_results["reference_data"]
                println(io, "Reference Data Downloaded:")
                println(io, "  Genome files: $(length(ref_data.genome_files))")
                println(io, "  CDS files: $(length(ref_data.cds_files))") 
                println(io, "  Protein files: $(length(ref_data.protein_files))")
                println(io, "")
            end
            
            if haskey(workflow_results, "observations")
                obs = workflow_results["observations"]
                println(io, "Simulated Observations Generated:")
                println(io, "  DNA reads: $(length(obs.dna))")
                println(io, "  RNA reads: $(length(obs.rna))")
                println(io, "  Protein reads: $(length(obs.protein))")
                println(io, "")
            end
            
            if haskey(workflow_results, "assembly_results")
                asm_results = workflow_results["assembly_results"]
                println(io, "Qualmer Assembly Results:")
                
                for (seq_type, result) in asm_results
                    println(io, "  $seq_type:")
                    println(io, "    Contigs: $(length(result.contigs))")
                    println(io, "    FASTQ contigs: $(length(result.fastq_contigs))")
                    if haskey(result.assembly_stats, "quality_preserved")
                        println(io, "    Quality preserved: $(result.assembly_stats["quality_preserved"])")
                    end
                    if haskey(result.assembly_stats, "mean_quality")
                        println(io, "    Mean quality: $(round(result.assembly_stats["mean_quality"], digits=2))")
                    end
                    if haskey(result.assembly_stats, "mean_coverage")
                        println(io, "    Mean coverage: $(round(result.assembly_stats["mean_coverage"], digits=2))")
                    end
                end
            end
            
            println(io, "")
            println(io, "=== Qualmer Assembly Features Demonstrated ===")
            println(io, "✓ Quality score propagation through assembly graphs")
            println(io, "✓ Consensus quality calculation from multiple observations")
            println(io, "✓ FASTQ output format preserving per-base quality")
            println(io, "✓ Iterative Viterbi algorithm for optimal path finding")
            println(io, "✓ Probabilistic walks for graph traversal")
            println(io, "✓ Heaviest path algorithm (highest confidence Eulerian paths)")
            println(io, "✓ Multi-sequence type support (DNA, RNA, amino acids)")
            println(io, "✓ Advanced bubble and repeat resolution")
        end
        
        @info "Workflow summary saved: $summary_file"
        
    else
        @info "Skipping assembly steps (run_assembly=false or no reference data)"
    end
    
    @info "=== Viroid Assembly Workflow Complete ==="
    @info "Results saved to: $outdir"
    
    return (
        viroid_name = viroid_name,
        output_directory = outdir,
        reference_data = get(workflow_results, "reference_data", nothing),
        observations = get(workflow_results, "observations", nothing), 
        assembly_results = get(workflow_results, "assembly_results", nothing),
        workflow_summary = joinpath(outdir, "viroid_assembly_workflow_summary.txt")
    )
end
...
"""
function _simulate_fastq_reads_from_sequence(sequence, identifier::String;
                                           coverage::Int=10,
                                           read_length::Int=150,
                                           error_rate::Float64=0.01,
                                           sequence_type::String="DNA")
    
    sequence_str = string(sequence)
    seq_length = length(sequence_str)
    ...
    num_reads = max(1, div(seq_length * coverage, read_length))
    ...
    for i in 1:num_reads
        ...
        if seq_length <= read_length
            start_pos = 1
            actual_read_length = seq_length
        else
            start_pos = rand(1:(seq_length - read_length + 1))
            actual_read_length = read_length
        end
        
        # Extract read sequence
        read_seq = sequence_str[start_pos:start_pos + actual_read_length - 1]
        
        # Add simulated errors
        if error_rate > 0.0
            read_seq = _add_simulated_errors(read_seq, error_rate, sequence_type)
        end
        
        # Generate quality scores (simulate high-quality reads with some variation)
        quality_scores = UInt8[]
        for j in 1:length(read_seq)
            if rand() < error_rate
                # Lower quality for error positions
                quality = rand(10:25)
            else
                # High quality for correct positions  
                quality = rand(35:40)
            end
            push!(quality_scores, UInt8(quality))
        end
        
        # Convert quality scores to ASCII
        quality_string = String(Char.(quality_scores .+ UInt8(33)))
        
        # Create FASTQ record
        read_id = "$(identifier)_sim_read_$(i)_pos_$(start_pos)"
        fastq_record = FASTX.FASTQ.Record(read_id, read_seq, quality_string)
        push!(reads, fastq_record)
    end
...
"""
function _add_simulated_errors(sequence::String, error_rate::Float64, sequence_type::String)
    if error_rate <= 0.0
        return sequence
    end
    ...
    # Define alphabets for different sequence types
    ...
    for i in 1:length(seq_chars)
        if rand() < error_rate
            ...
            original_char = seq_chars[i]
            available_chars = filter(c -> c != original_char, alphabet)
            if !isempty(available_chars)
                seq_chars[i] = rand(available_chars)
            end
        end
    end
