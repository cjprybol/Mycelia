...
function prepare_binning_test_inputs(;
    ...
    #
    # Gated on OWNERSHIP, not on the default outdir. NON-COLLISION is the
    # safety condition -- a concurrent peer's in-flight chunks are equally
    # unresumable and equally match the predicate. The bam name here is fixed
    # ("contigs.minimap2.sorted.bam"), so two callers sharing an explicit
    # `outdir` derive one split prefix, and an unguarded entry-time sweep would
    # unlink a live peer's chunks -- the unlink succeeds while minimap2 holds
    # the fd, so the victim does not error, it silently loses index parts.
    #
    # The previous `outdir === nothing` gate had the same defect as
    # `minimap_merge_map_and_split`'s: when it held, `inputs_dir` was a fresh
    # `mktempdir()` (see above), so the swept directory was empty by
    # construction; when it did not hold, the sweep was skipped. It could never
    # reclaim anything. The ownership sidecar answers the collision question
    # directly, so the sweep can run on both paths.
    ...
    if !isfile(mapping.outfile) || filesize(mapping.outfile) == 0
        ...
        #
        # Tracked before the run because the sweep above trusts the sidecar: a
        # site that maps WITHOUT tracking leaves chunks a later sweep reads as
        # `:absent` and reclaims while they are still in flight.
        Mycelia.register_minimap_split_temp_atexit()
        Mycelia.track_minimap_split_prefix(mapping.split_prefix)
        try
            run(mapping.cmd)
        finally
            Mycelia.cleanup_minimap_split_temps(mapping.split_prefix)
            Mycelia.untrack_minimap_split_prefix(mapping.split_prefix)
        end
    end
