...
"""
    score_protein_inventory_candidates(df::DataFrames.DataFrame; ...)

Assign a reusable heuristic confidence score to protein inventory candidates
based on annotation specificity, expected length, source quality, and fragment
status.

This is intended for broad sequence acquisition workflows where an initial pull
needs a second-pass curation stage before manual review.
"""
function score_protein_inventory_candidates(df::DataFrames.DataFrame;
        description_col::Symbol = :Description,
        length_col::Symbol = :Length,
        fragment_col::Symbol = :Is_Fragment,
        source_col::Union{Nothing, Symbol} = :Source,
        reviewed_col::Union{Nothing, Symbol} = :Reviewed,
        expected_length::Int,
        strict_length_range::UnitRange{Int},
        exact_patterns::Vector{Regex} = Regex[],
        include_patterns::Vector{Regex} = Regex[],
        penalty_patterns::Vector{Regex} = Regex[],
    ...
    for row in eachrow(scored)
        description = lowercase(strip(string(row[description_col])))
        length_value = Int(row[length_col])
        is_fragment = Bool(row[fragment_col])

        this_annotation_score = 0
        any(occursin.(exact_patterns, Ref(description))) && (this_annotation_score += 3)
        this_annotation_score += count(pattern -> occursin(pattern, description), include_patterns)
        this_annotation_score -= count(pattern -> occursin(pattern, description), penalty_patterns)
        this_annotation_score -= 3 * count(pattern -> occursin(pattern, description), exclude_patterns)

        deviation = abs(length_value - expected_length) / expected_length
        this_length_score = if length_value in strict_length_range
            3
        ...
        this_source_score = 0
        if source_col !== nothing
            this_source_score += get(source_weights, string(row[source_col]), 0)
        end
        if reviewed_col !== nothing
            reviewed_value = lowercase(strip(string(row[reviewed_col])))
            occursin("reviewed", reviewed_value) && (this_source_score += 1)
        end

        this_fragment_penalty = is_fragment ? -3 : 0
        this_total_score = this_annotation_score + this_length_score + this_source_score + this_fragment_penalty
        this_is_high_confidence = this_total_score >= score_threshold &&
                                  !is_fragment &&
                                  length_value in strict_length_range &&
                                  !any(pattern -> occursin(pattern, description), exclude_patterns)

        push!(annotation_score, this_annotation_score)
        push!(length_score, this_length_score)
        push!(source_score, this_source_score)
        push!(fragment_penalty, this_fragment_penalty)
        push!(total_score, this_total_score)
        push!(is_high_confidence, this_is_high_confidence)
    end

    scored[!, :Annotation_Score] = annotation_score
    scored[!, :Length_Score] = length_score
    scored[!, :Source_Score] = source_score
    scored[!, :Fragment_Penalty] = fragment_penalty
    scored[!, :Curation_Score] = total_score
    scored[!, :Is_High_Confidence] = is_high_confidence
    return scored
end
