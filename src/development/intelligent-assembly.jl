...
function calculate_assembly_reward(graph, corrections_made::Int, k::Int,
    ...
    if previous_metrics !== nothing
        accuracy_improvement = current_metrics[:overall_accuracy] -
                               previous_metrics[:overall_accuracy]
...
function mycelia_assemble(reads::Vector{<:FASTX.FASTQ.Record};
    ...
    while k <= max_k && iteration <= max_iterations
        ...
        previous_metrics = length(reward_history) > 0 ?
                           accuracy_metrics_history[k_progression[end]] : nothing
        ...
        should_continue = should_continue_k(graph, corrections_made, k) &&
                          k_iterations < max_k_iterations
    ...
    return finalize_assembly(
        assembly_graphs, k_progression, total_corrections, total_runtime,
...
function finalize_assembly(assembly_graphs::Dict{Int, Any}, k_progression::Vector{Int},
        ...
        accuracy_metrics_history::Dict{Int, Dict{Symbol, Float64}} = Dict{
            Int, Dict{Symbol, Float64}}();
        verbose::Bool = true)
    if verbose
    ...
    total_kmers = sum(length(MetaGraphsNext.labels(graph))
    for graph in values(assembly_graphs))
    ...
    reward_stats = if !isempty(reward_history)
        Dict(
            ...
            :reward_improvement => length(reward_history) > 1 ?
                                   reward_history[end] - reward_history[1] : 0.0
        )
    else
        Dict(
            :mean_reward => 0.0,
...
function test_intelligent_assembly()
    try
        ...
        sample_reads = [FASTX.FASTQ.Record("read$i", seq, repeat("H", length(seq)))
...
function mycelia_assemble(input_file::Union{String, Vector{String}};
    ...
    verbose &&
        println("📊 Total bases: $(sum(length(FASTX.sequence(r)) for r in all_reads))")
    println()
