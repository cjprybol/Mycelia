...
"""
function write_pantools_annotation_locations_file(
        annotation_by_genome::AbstractDict{<:Integer, <:AbstractString};
        outfile::Union{Nothing, AbstractString} = nothing)
...
function pantools_add_annotations(db::Union{PantoolsDB, AbstractString};
    ...
    return (; db = PantoolsDB(db_dir),
        annotation_overview = isfile(overview) ? overview : nothing)
end
...
function pantools_metrics(db::Union{PantoolsDB, AbstractString};
    ...
    return (; db = PantoolsDB(db_dir),
        metrics_per_genome = isfile(metrics_per_genome) ? metrics_per_genome : nothing)
end
...
function pantools_pangenome_structure(db::Union{PantoolsDB, AbstractString};
    ...
    return (; db = PantoolsDB(db_dir),
        pangenome_structure_dir = isdir(outdir) ? outdir : nothing)
end
...
function pantools_retrieve_regions(db::Union{PantoolsDB, AbstractString};
    ...
    return (; db = PantoolsDB(db_dir),
        retrieval_dir = isdir(retrieval_dir) ? retrieval_dir : nothing)
end
...
"""
function write_pantools_functions_locations_file(
        functions_by_genome::AbstractDict{<:Integer, <:AbstractString};
        outfile::Union{Nothing, AbstractString} = nothing)
...
function pantools_function_overview(db::Union{PantoolsDB, AbstractString};
    ...
    return (; db = PantoolsDB(db_dir),
        function_overview_per_group = isfile(overview) ? overview : nothing)
end
...
function pantools_go_enrichment(db::Union{PantoolsDB, AbstractString},
    ...
    return (;
        db = PantoolsDB(db_dir), go_enrichment_csv = isfile(go_csv) ? go_csv : nothing)
end
...
function pantools_functional_classification(db::Union{PantoolsDB, AbstractString};
    ...
    return (; db = PantoolsDB(db_dir),
        functional_classification_dir = isdir(outdir) ? outdir : nothing)
end
...
function pantools_msa(db::Union{PantoolsDB, AbstractString},
    ...
    return (; db = PantoolsDB(db_dir),
        alignments_dir = isdir(alignments_dir) ? alignments_dir : nothing)
end
